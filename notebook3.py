...
def main():
    ...
    mox.file.copy_parallel('obs://ghost-story/ghost/nlpdata/model/', '/cache/model')
    ...
    parser.add_argument('--model_config', default='nlpdata/config/model_config_small.json', type=str, required=False,
                        help='模型参数')
    parser.add_argument('--tokenizer_path', default='nlpdata/cache/vocab_small.txt', type=str, required=False,
                        help='词表路径')
    parser.add_argument('--model_path', default='nlpdata/model/final_model', type=str, required=False, help='模型路径')
    parser.add_argument('--prefix', default='鬼怪', type=str, required=False, help='生成文章的开头')
    ...
    tokenizer = BertTokenizer('/cache/cache/vocab_small.txt')
    model = GPT2LMHeadModel.from_pretrained('/cache/model')
    model.to(device)
    model.eval()
